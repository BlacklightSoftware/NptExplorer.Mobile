...
namespace NptExplorer.Mobile.Controls
{
    public partial class Chip : Frame
    {
        ...
        {
            if (this.ForceWidthToAtLeastHeight && !this._isForcingWidth && height - width >= 1.0)
            {
                this._isForcingWidth = true;

                this.WidthRequest = height;
            }
            else
            {
                this._isForcingWidth = false;
